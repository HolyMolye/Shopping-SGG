  ...
  <div class="clearfix selector">
    <div class="type-wrap logo">
      ...
      <div class="value logos">
        <ul class="logo-list">
          <!-- 这下面tradeMatkHandler(trademark)后面的括号是要传参，自定义事件子组件给父组件传的参 -->
    ...
    <!-- 平台售卖属性的地方 -->
    <div class="type-wrap" v-for="(attr, index) in attrList" :key="attr.attrId">
      <!-- 平台售卖属性：比如颜色 -->
...
export default {
  ...
  methods: {
    ...
    tradeMatkHandler(trademark) {
      //点击了品牌中的品牌名称（苹果），还是需要整理参数，向服务器发请求获取相应的数据进行展示
      //是在那个组件中发请求到服务器呢？父组件search
      //为什么？因为父组件中searchParams参数是带给服务器的参数，子组件把你的点击的品牌信息，需要给父组件传过去
      //子=>父  自定义事件
      this.$emit('trademarkInfo', trademark)//('父组件的回调名称',带什么过去)
    },

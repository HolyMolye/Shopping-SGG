...
export default {
    ...
    //组件挂载完毕的地方
    mounted() {
        //之前在ListContainer写Swiper的时候，在mounted当中是不可以的，但是为什么现在在这里可以呢？
        //ListContainer里书写轮播图的时候，是在当前组件内部发请求、动态渲染结构【前台至少服务器数据需要回来】，因此当年的写法在这里不行
    ...
    watch: {
        /*list: {
            //这边的轮播图代码虽然写在mounted当中可以，但是为了好拆分组件，所以写在watch，目的是能跟ListContainer下的代码一样
            //立即监听：不管你数据有没有变化，我上来立即监听一次
            //为什么watch监听不到list，因为这个数据从来没有发生变化(数据是父亲给的[props]，父亲给的时候就是一个对象，对象里面该有的数据都是有的)
            immediate: true,
            handler() {
